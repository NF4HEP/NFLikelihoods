...
def graph_execution(ndims,trainable_distribution, X_data,n_epochs, batch_size, n_disp,path_to_results,load_weights=False,load_weights_path=None,lr=.001,patience=30,min_delta_patience=0.001,reduce_lr_factor=0.2,seed=0):
    ...
    monitor='val_loss', min_delta=min_delta_patience, patience=patience*2, verbose=1,
    mode='auto', baseline=None, restore_best_weights=True
     )
    #reducelronplateau
    reducelronplateau=tf.keras.callbacks.ReduceLROnPlateau(
    monitor="val_loss", min_delta=min_delta_patience, patience=patience, verbose=1,
    factor=reduce_lr_factor, mode="auto", cooldown=0, min_lr=lr/100000000
     )
...
def graph_execution_iter(ndims,trainable_distribution, X_data,n_epochs, batch_size, n_disp,path_to_results,load_weights=False,load_weights_path=None,lr=.001,patience=30,min_delta_patience=0.001,reduce_lr_factor=0.2,seed=0):
    #Utils.reset_random_seeds(seed)
    #log_file_train=open('train_log.txt','w')
    #log_file_train.write('### '+str(ndims)+' ### \n')
    #log_file_train.close()
    ...
    reducelronplateau=tf.keras.callbacks.ReduceLROnPlateau(
    monitor="val_loss", min_delta=min_delta_patience, patience=patience, verbose=1,
    factor=reduce_lr_factor, mode="auto", cooldown=0, min_lr=lr/10000
     )
    # Display the loss every n_disp epoch
    epoch_callback = LambdaCallback(
        on_epoch_end=lambda epoch, logs:
